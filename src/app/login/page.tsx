  ...
  return (
    <div className="flex min-h-[calc(100vh-4rem)] items-center justify-center px-4 py-12 sm:px-6 lg:px-8 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-primary/10 via-background to-background">
      <div className="w-full max-w-md space-y-8 p-8 rounded-2xl glass border border-border/40 glow-purple">
        ...
        {error && (
          <div className="flex items-start gap-2 rounded-xl bg-destructive/10 border border-destructive/20 p-3 text-sm text-destructive font-medium">
            <AlertCircle className="h-4 w-4 mt-0.5 shrink-0" />
            <span>
              {error === "AccessDenied"
                ? "This Google account isn't associated with any FlowDesk organization. Ask your team owner for an invite link, or create a new organization."
        ...
        <div className="mt-8 space-y-6">
          {session?.user?.id ? (
            // Auth.js links a new OAuth account to whichever user is currently signed
            // in when the provider callback fires (inside @auth/core, before our own
            // signIn/jwt callbacks run) — so retrying "Continue with Google" here while
            // still signed in as the rejected/previous account would silently attach
            // the next account picked to this session's user instead of evaluating it
            // on its own. Require an explicit sign-out first.
            <form
              action={async () => {
                "use server";
            ...
            >
              ...
              >
                ...
                Sign out to try a different account
              </button>
            </form>
          ) : (
            <form
